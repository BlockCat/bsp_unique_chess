...
class Test : public mcbsp::BSP_program {
    ...
    void spmd() {            
		...
		while (currentDepth < max_depth) {
			...
			if (pid == 0) debug_print("(%d) distribution size: %lu\n", pid, distribution.size());
		}		
		...
		int localWork = corePositionsBlackToMove[pid].size() + corePositionsWhiteToMove[pid].size();
		
		int totalWork = merge(totalSum);		
		int total = merge(localWork);
	...
	PositionSet execute(PositionSet &distribution) {
		...
		for (auto it = distribution.begin(); it != distribution.end(); it++) {			
			PositionSet children = GetCompressedChildren(*it);
	...
	vector<PositionSet> redistribute(PositionSet &list, vector<PositionSet> &cores) {		
		...
		for (auto it = list.begin(); it != list.end(); it++) {			
			CoreId core = HashBoard(*it) % nprocs;
			// If we haven't seen this one yet, send it.
			if (cores[core].insert(*it).second) {				
				corePositionSets[core].insert(*it);
			}
		}
	...
	CoreId createBoardHash(CompressedPosition &position) {
		
		ChessRules* board = decompress(position);
		int nprocs = bsp_nprocs();		
		if (nprocs == 1) return 0;

		int bucket = nprocs / 2;
		bool b1 = board->squares[Square::b1] == 'N';
		bool g1 = board->squares[Square::g1] == 'N';
		bool b8 = board->squares[Square::b8] == 'n';
		bool g8 = board->squares[Square::g8] == 'n';
		...
		if ((b1 && g1 && b8 && g8)) {			
			return (
				...
				position.storage[21] +
				position.storage[22] +
				position.storage[23]) % bucket;
		}

		string concatenation = "";

		for (int i = Square::a8; i < Square::h1; i++) {
			char piece = board->squares[i];
			if (piece == 'n') {
				//chessPieces.push_back(make_tuple(i, piece));
				concatenation += to_string(i) + ": n";
			}
			if (piece == 'N') {
				//chessPieces.push_back(make_tuple(i, piece));
				concatenation += to_string(i) + ": N";
		...
		size_t str_hash = hash<std::string>{}(concatenation);

		// Skip the first core that handles all chess moves where knights have not been moved yet.
		// Position where knights have not been moved will be much larger than positions where knights have moved.
		// That is, with a bounded depth.
		CoreId core = str_hash % (nprocs - bucket);
		return core + bucket;		
	}
